#-----------------------------------------------------------------------------------------------------------
# This code performs confirmatory factor analysis on models resulting from EFA performed on the
# Serendipity'2018 dataset, which has generously been made available by the authors:
...
Ser.model <- ' relevance     =~ rating + s7
              novelty        =~ s1 + s2
              unexpectedness =~ s3 + s4 + s5 + s6
              serendipity    =~ s7 + s8'

# specification of the alternative model
Ser.model.alt <- ' relevance     =~ rating + s7
              surprise        =~ s1 + s2 + s3 + s4 + s5 + s6
              serendipity    =~ s7 + s8'

# single factor model
Ser.model.one <- ' singlefactor =~ rating + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 '
...
# This script has been organised as a "buffett": general definitions and data have been loaded.
# You are expected explore possible analyses and comparisons:

# -- to fit a confirmatory model to the data:
# postulated <- induceModel(Ser.model, serData)
